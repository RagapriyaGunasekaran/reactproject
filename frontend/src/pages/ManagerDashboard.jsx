...
const ManagerDashboard = () => {
    ...
    useEffect(() => {
        const loadDashboardData = async () => {
            try {
                ...
                // Fetch Employees
                const empRes = await axios.get('http://localhost:5000/api/notifications/employees', config);
                setEmployees(empRes.data);
                
                // Fetch History
                const histRes = await axios.get('http://localhost:5000/api/notifications/history', config);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const config = { headers: { Authorization: `Bearer ${token}` } };
            const res = await axios.post('http://localhost:5000/api/notifications/send', task, config);
